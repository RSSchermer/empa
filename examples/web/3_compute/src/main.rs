...
use empa::command::{Draw, RenderPassDescriptor, DispatchWorkgroups};
...
use empa::compute_pipeline::{ComputePipelineDescriptorBuilder, ComputeStage, ComputeStageBuilder};
use std::ops::Deref;
...
async fn render() -> Result<(), Box<dyn Error>> {
    ...
    let readback_buffer: Buffer<[u32], _> = device.create_buffer(vec![0u32; 1024], buffer::Usages::map_read().and_copy_dst());
...

    {
        ...
        console::log!("Numbers squared on the GPU:", format!("{:#?}", data.deref()));
    }
